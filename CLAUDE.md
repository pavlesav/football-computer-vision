...
8. **Human review loop (`src/review_ui.py`)** — per (match, half): `python -m src.review_ui --match X --half N` generates `output/review/{slug}_p{N}_review.html` (self-contained: crop galleries for top event-participating metas + all jersey-confident metas, OCR prefills, localStorage autosave). Human types shirt numbers (names optional), clicks Export, then `--apply <file>` writes `data/identities/{slug}_p{N}.json`; re-run `run_match` to rebuild events+report with unified `#N`/named players (numbers get the same period-independent id scheme as OCR'd ones). ~10 min/half closes the jersey-OCR recall gap for everyone who matters. sut-mla pages are already generated. After batch: correction UI v2 scoped by real per-match error volumes (ID-swap adjudication using team_repair + jersey vote-split flags).
