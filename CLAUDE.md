...
│   ├── run_match.py                   # Orchestrator: perception both halves → stabilize → jersey OCR → oracle → events → report (resumable)
│   ├── run_batch.py                   # Sequential Tier-1 queue driver (verified home_team map; launch detached — see docstring)
│   ├── review_ui.py                   # Human review: self-contained HTML crop galleries → numbers/names JSON → data/identities/
...
7. **Tier-1 batch RUNNING (launched detached 2026-07-03)** via `src/run_batch.py` → `src/run_match.py` per match (perception both halves → stabilize → jersey OCR → oracle → events → report; resumable via `output/match_runs/{slug}_status.json`; progress in `output/match_runs/batch_tier1.{out,err}`). Queue: jed-ars(1), jez-ars(0), jez-jed(0), sut-pet(0), mla-bud-2(1), dec-mla(1) — home_team ids visually verified from scoreboard chips + classifier kit crops. ~10h/match, one GPU job at a time. If the driver dies, relaunch the same command — completed steps skip.
8. **Human review loop (`src/review_ui.py`)** — per (match, half): `python -m src.review_ui --match X --half N` generates `output/review/{slug}_p{N}_review.html` (self-contained: crop galleries with mm:ss timestamps + avg-position mini-pitch, SofaScore lineup side panel from `data/lineups/{slug}.json` with click-to-fill and number→name autocomplete, localStorage autosave). **Cards are one RAW TRACK each** (top-40 by event participation + ≤15 jersey-OCR-read tracks; `--metas` = legacy grouped mode): the first real labeling session proved `consolidate_tracks` merges different players (every mixed card was a 9-126-member meta; dec-mla 6-7/10 cards mixed, post-fix ~9/10 single-player) — same-player fragments unify via the shared number+team export id instead, so consolidation never touches identity. Human types shirt numbers (name auto-fills; lineup click pins the true team over the classifier chip), ticks "2+ players mixed" for in-track swaps (excluded from naming, recorded as split candidates), Export → `--apply <file>` → `data/identities/{slug}_p{N}.json` → re-run `run_match`. Goal-oracle validated 7/7 exact vs SofaScore real results (after the derive_goals persistence check killed dec-mla's phantom reverted goal). Next: correction UI v2 scoped by mixed-flag volume (ID-swap adjudication/splitting).
